'use strict';
var goldSaints = JSON.parse(localStorage['cavaleiros'] ||
...
$('#listacavaleiros img').hover(function(e){
  ...
  var carac = [
    'Nome: ' + nome,
    'Peso: ' + arredondamento(lbtoKg(cavaleiro.pesoLb)) + 'kg',
    'Altura: ' + cmtoMeters(cavaleiro.alturaCm) + 'm',
    'Signo: ' + cavaleiro.signo,
    'Local nascimento/treinamento: ' + cavaleiro.localNascimento  + '/' + cavaleiro.localTreinamento,
    'Golpes: ' + cavaleiro.golpes.join(','),
    'Tipo Sanguineo: ' + cavaleiro.tipoSanguineo,
    'Data nascimento: ' + toBrDate(cavaleiro.dataNascimento),
  ];

  var $informacoes = $('#informacoes');
  for (var campo in carac) {
    $informacoes.append($('<li>').text(carac[campo]));
  };
