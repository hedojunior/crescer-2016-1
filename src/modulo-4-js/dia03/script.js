...
goldSaints.forEach(function(i){
  var img = i.imagens[0].url;
  var $img = $('<img>').attr('src',img).attr('alt',i.nome).attr('id',i.id);
  $('#listacavaleiros').append($('<li>').append($img));
});

$('#listacavaleiros img').hover(function(e){
    var self = $(this);
    var nome = self.attr('alt');
    var cavaleiro = goldSaints.filter(function(elem) {
          return elem.id === parseInt(self.attr('id'));
    })[0];
    var peso ='Peso: ' + arredondamento(lbtoKg(cavaleiro.pesoLb)) + 'kg';
    var altura ='Altura: ' + cmtoMeters(cavaleiro.alturaCm) + 'm';
    var signo ='Signo: ' + cavaleiro.signo;
    var localnasctreinamento ='Local nascimento/treinamento: ' + cavaleiro.localNascimento  + '/' + cavaleiro.localTreinamento;
    var golpes = 'Golpes: ' + cavaleiro.golpes.join(',');
    var tipoSangue ='Tipo Sanguineo: ' + cavaleiro.tipoSanguineo;
    $('#informacoes').append($('<li>').append($('<h2>').text( nome )));
    $('#informacoes').append($('<li>').text(peso));
    $('#informacoes').append($('<li>').text(altura));
    $('#informacoes').append($('<li>').text( signo ));
    $('#informacoes').append($('<li>').text( localnasctreinamento ));
    $('#informacoes').append($('<li>').text( tipoSangue ));
    $('#informacoes').append($('<li>').text( golpes ));

}, function(e){
  $('#informacoes').attr('display','none');
  $('#informacoes').empty();
});
